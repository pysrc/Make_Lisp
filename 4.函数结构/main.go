...
func main() {
	...
	/**
		// 斐波拉契递归测试
		c := Code{}
		c.Init("(fn f [x] (if (<= x 2) 1 (+ (f (- x 1)) (f (- x 2)))))")
		s := c.Read_Root()
		fmt.Println(s)
		fmt.Println(Eval(s, &EnvMap))
		c1 := Code{}
		c1.Init("(f 1)")
		fmt.Println(Eval(c1.Read_Root(), &EnvMap))
		c2 := Code{}
		c2.Init("(f 2)")
		fmt.Println(Eval(c2.Read_Root(), &EnvMap))
		c3 := Code{}
		c3.Init("(f 3)")
		fmt.Println(Eval(c3.Read_Root(), &EnvMap))
		c4 := Code{}
		c4.Init("(f 4)")
		fmt.Println(Eval(c4.Read_Root(), &EnvMap))
		c5 := Code{}
		c5.Init("(f 5)")
		fmt.Println(Eval(c5.Read_Root(), &EnvMap))
		c6 := Code{}
		c6.Init("(f 6)")
		fmt.Println(Eval(c6.Read_Root(), &EnvMap))
	    **/
	/*
		// 函数相互调用测试
		c := Code{}
