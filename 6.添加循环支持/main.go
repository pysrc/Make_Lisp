	...
	for v != ")" && v != nil {
		if v == "(" {
			...
			for v != "}" && v != nil {
				if v == "(" {
					self.Next()
					expr.Val = append(expr.Val, self.read_list())
				}
				v = self.Next()
			}
			lt = append(lt, expr)
	...
	*/
	...
	case []Object:
		...
		case "for": // 循环语句(for (bool expr) [(expr1) (expr2) (expr3) ...])
			eprlst := v[2].(ExprList).Val // 循环体
			for Eval(v[1], env).(bool) {  // v[1] 是循环判断结构
				for _, epr := range eprlst { // 执行循环体
					Eval(epr, env)
				}
			}
		default:
...
func ExeFile(cmd string) { // 执行文件
	...
	for _, j := range cmds {
		...
		if pr != "" {
			...
			c.Init(pr)
			s := c.Read_Root()
			if strings.Contains(pr, "set") || strings.Contains(pr, "fn") || strings.Contains(pr, "for") {
				Eval(s, &EnvMap)
			} else {
