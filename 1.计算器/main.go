...
func Apply(v []Object, fn func(Object) Object) []Object { // 将函数fn 应用到列表每一项
	var res []Object
	for _, j := range v {
		res = append(res, fn(j))
	}
...
func Eval(tree Object) Object { // 计算表达式
	switch tree.(type) {
	case []Object:
		v, _ := tree.([]Object)
		if len(v) < 2 {
		...
		// 取出操作符及其对应的函数
		f, ok := OpMap[v[0].(string)].(func([]Object) Object)
		if ok {
			return f(Apply(v[1:], Eval))
		}
	case Object:
		return tree
	}
